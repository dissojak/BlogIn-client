...
export class HomeComponent implements OnInit  {
  ...
  ngOnInit(): void {
    ...
    this.userService.getPopularUser().subscribe(res=>{
      this.popularUsers=res
      this.syncPopularUsers()
    })
...
onSubmit(){
  ...
  followUser(id:string){
    ...
    this.userService.followUser(id).subscribe(_=>{
      if (!this.user.followings.includes(id)) {
        this.user.followings.push(id)
      }
      const target = this.popularUsers.find((u:any)=>u._id === id)
      if (target) {
  ...
  private syncPopularUsers(){
    if (!this.user || !this.popularUsers?.length) {
      return
    }
    this.popularUsers = this.popularUsers.map((user:any)=>({
      ...user,
      isFollowed: this.user.followings?.includes(user._id)
    }))
  }
}
