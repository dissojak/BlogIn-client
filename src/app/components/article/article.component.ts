...
rateArticle(stars :number){
 ...
 this.nbStars=stars
 this.articleService.ratingArticle(this.postId , stars).subscribe(res=>{
 })
}
...
saveComment(){
  ...
  const content = this.comElentRef?.nativeElement?.innerText?.trim()
  if (!content) {
    return
  }

  this.commentService.saveComment(this.user._id ,this.user.name ,this.user.image, content , this.postId).subscribe(res=>{
    ...
    this.comElentRef.nativeElement.innerText = ''
    this.hideFollowers()
