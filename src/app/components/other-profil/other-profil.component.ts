...
moveDiveRight(){
  ...
  followUser(){
   this.userService.followUser(this.user._id).subscribe(res=>{
    // refresh user from backend to ensure followers count is accurate
    this.userService.getUserById(this.userId).subscribe(user=>{
      this.user = user;
  ...
  followother(id :string , followers : string[]){
    this.userService.followUser(id).subscribe(res=>{
     // after following another user, re-fetch the list of followers/followings
     this.userService.getFollowings(followers).subscribe(res2=>{
       this.followersOrFollowings = res2;
